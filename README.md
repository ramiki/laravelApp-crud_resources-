...
## laravel tester :  version 8.x

# pre-config :

- Laravel Installation : composer create-project --prefer-dist laravel/laravel crud
- cp : .env.example to .env  
- composer install : to install all packages find in composer.jon
...
- creat controller and his model with resource methode : php artisan make:controller FormController --model=Form --resource 
    ( app\Http\Controllers\FormController.php with resource & app\Models\Form.php ) 
...
- add request for validation :  php artisan make:request formrequest    ( config formrequesty.php ) and call it ex : in the methode (store) request argument (class / hint) of the controller :
...
- add policy and auhorization : php artisan make:policy formPolicy --model=Form    ( config formPolicy.php )
                                ...
                                call it ex : in the controller (edite methode): $this->authorize('update' , $form);
                                customize the error page ex : 403 HTTP status codes, create a resources/views/errors/403.blade.php view template. This view will be rendered on all 403 errors
                                generated by your application. The views within this directory should be named to match the HTTP status code they correspond to( 401 , 404 ....) .
...
- auth by ui auth package :
            ...
            install :
			    ...
			    php artisan ui bootstrap --auth   // can use startup templat of bootstrap , vue or react
                // ref :https://medium.com/@agavitalis/php-artisan-make-auth-command-is-not-defined-laravel-6-b51adcc6356d
			    npm install
			    npm run dev

            usage :  work directly after install . for more about ui authentication : ref https://laravelarticle.com/laravel-8-authentication-tutorial
...
- JWT api auth : ref : https://www.positronx.io/laravel-jwt-authentication-tutorial-user-login-signup-api/   
                       https://pusher.com/blog/laravel-jwt/#creating-our-routes 
                officiel JWT doc : https://jwt-auth.readthedocs.io/en/develop/       
                app tools  : https://jwt.io/ 
            auth guard fixed : adding 'api' to all auth() mehode of authcontroller : auth('api')
...
# php artisan :  
  ...
  list of tested cmd :
      ...
      ui:auth       // install ui auth package with out template view
                       ...
                       For example, Laravel ships with a session guard which maintains state using session storage and cookies. ( jwt for api's with token auth )
      ...
      vendor:publish// when a users package's execute the vendor:publish command, his files will be copied to the specified location.
...
- **** for more infos see the doc ***
    ...
    - To obtain an instance of the current HTTP request via dependency injection, you should type-hint the Illuminate\Http\Request class on your route closure or controller method.
        The incoming request instance will automatically be injected by the Laravel service container , ex : public function store(Request $request)  { ... }
    ...
    - db 'engine' as 'InnoDB' : Edit /config/database.php file, search for mysql entry and change:
        'engine' => null,    to  'engine' => 'InnoDB', This saves you from adding $table->engine = "InnoDB"; for each of your Schemas

    -  Eloquent scopes : Re-using query constraints in Laravel Eloquent - Global vs. Local scopes ( ref https://www.amitmerchant.com/laravel-eloquent-global-local-scope )
        This can be useful to minimize the code while writing the query and is an easy way to make sure every query for a given model receives certain constraints.
    ...
    - role : concept is to Adding Role column (ex : is_admin) to table "ex : users table" after that we creat a middeleware 'admin' and cal it in route
        ...
        for multi-roles we can creat a table ( role = admin = 1 , manager = 2 , user = 3 ... ) spared from user table , and related it with a column name in user table 'role = 1|2|3..' 

    - authentication : verifies the identity of a user or service before granting them access and passed to guards for more check , (see routes/app/http/kernel) 
    ...
    - authorization : In Laravel, the authorization system is built on top of the concept of permissions. Laravel provides various 
        ...
        You dont need to choose between exclusively using gates or exclusively using policies when building an application. 
    ...
    - mail fix ssl bug laravel 7 ! : 
        ...
        routines:tls_process_server_certificate:certificate verify failed
            ...
            fix 2 :  go to the file vendor\swiftmailer\lib\classes\Swift\Transport\StreamBuffer.php and comment out the code 
                $options = []; and paste the code : $options['ssl'] = array('verify_peer' => false, 
